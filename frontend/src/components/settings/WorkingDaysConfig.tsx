...
const WorkingDaysConfig: React.FC<WorkingDaysConfigProps> = ({
        ...
        try {
          ...
          if (profileResponse.data.status === 'success') {
            ...
            setWorkingDaysData({
              ...
              working_hours: employee.working_hours || { start: '09:00', end: '17:00' },
  ...
  const validateConfig = (): string[] => {
    ...
    const startTime = new Date(`2000-01-01T${workingDaysData.working_hours.start}:00`);
    const endTime = new Date(`2000-01-01T${workingDaysData.working_hours.end}:00`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await api.put('/employees/me', {
        working_days: workingDaysData.working_days,
      ...
      addToast('error', error.response?.data?.message || 'Failed to save working days configuration');
    } finally {
      setIsSaving(false);
    }
  };
    ...
    return (
      <Card className={`p-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        <div className="animate-pulse">
          <div className={`h-4 rounded mb-4 ${darkMode ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className={`text-xs mt-2 ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
            Selected: {workingDaysData.working_days.length} day{workingDaysData.working_days.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        {validationErrors.length > 0 && (
          <div className={`p-3 rounded-lg ${darkMode ? 'bg-red-900/20 border-red-800' : 'bg-red-50 border-red-200'} border`}>
        ...
        {!hasChanges && !isLoading && (
          <div className={`p-3 rounded-lg ${darkMode ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'} border`}>
            <div className="flex items-center gap-2">
              <CheckCircle className="h-4 w-4 text-green-500" />
              <span className={`text-sm ${darkMode ? 'text-green-400' : 'text-green-800'}`}>
        ...
        <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          <p>• Working days configuration affects performance calculations and attendance tracking</p>
          <p>• Changes take effect immediately and will be used for future calculations</p>
          <p>• Your timezone setting ensures accurate time-based calculations</p>
        </div>
      </div>
    </Card>
  );
